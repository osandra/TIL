### 네트워크
- [HTTP 메서드 및 헤더](./Today/network/http.md)
- [DNS](./Today/network/dns.md)

### 운영체제
- [프로세스 & 스레드](./Today/os/process.md)
- [프로세스 스케줄링](./Today/os/process_sche.md)
- [동기화](./Today/os/synchronization.md)
- [데드락](./Today/os/deadlock.md)

### 웹
- [JWT](./Today/web/jwt.md)
- [URI, URL, URN](./Today/web/url.md)

### 알고리즘
- [플로이드워셜 알고리즘](./Today/Algorithm/concept/chap9_concept.md)
- [진수 변환](./Today/Algorithm/concept/진수변환.md)
- [기타 알고리즘](./Today/Algorithm/concept/partB.md)
- [비트 연산](./Today/Algorithm/concept/비트연산.md)
- [알고리즘 문제풀이](./Today/Algorithm/practice_algo/practice.md)

### 코틀린
- [코틀린 인 액션](./Today/Kotlin/kotlinInAction/kotlinInAction.md)

### [스위프트](./Today/Swift/swift.md)